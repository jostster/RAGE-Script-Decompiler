...
{
    internal class X64NativeFile : Dictionary<ulong, Native>
    {
        /// <summary>
        ...
        {
            using (var sr = new StreamReader(nativefile))
            {
                var o = (RootObject) new JsonSerializer().Deserialize(sr, typeof(RootObject));
                foreach (var ns in o.Values)
                foreach (var natives in ns.Value.ToObject<RootObject>().Values)
                {
                    var native = natives.Value.ToObject<Native>();
                    foreach (var p in native.Params)
                        p.UpdateFixed();
                    ...
                    {
                        uint jhash;
                        if (native.Joaat != "" && uint.TryParse(native.Joaat.Substring(2), NumberStyles.HexNumber,
                            CultureInfo.InvariantCulture, out jhash))
                        {
                            native.Hash = jhash;
                            this[jhash] = native;
                        }
                    ...
                    {
                        ulong hash;
                        if (ulong.TryParse(natives.Key.Substring(2), NumberStyles.HexNumber,
                            CultureInfo.InvariantCulture, out hash))
                        {
                            native.Hash = hash;
                            this[hash] = native;
                            if (native.Hashes != null)
                                foreach (var s in native.Hashes)
                                    if (ulong.TryParse(s.Substring(2), NumberStyles.HexNumber,
                                        CultureInfo.InvariantCulture, out hash) && hash != 0 && !ContainsKey(hash))
                                        Add(hash, native);
                        }
                    }
                }
        ...
        public string GetNativeInfo(ulong hash)
        {
            ...
            var dec = native.ReturnParam.StackType.ReturnType() + native.Display + "(";
            if (native.Params.Count == 0)
                return dec + ");";
            for (var i = 0; i < native.Params.Count; i++)
            ...
            lock (Program.ThreadLock)
            {
                if (TryGetValue(hash, out native))
                {
                    ...
                    */
                    ...
                    {
                        Console.WriteLine(
                            "Native Argument Mismatch: " + name + " " + pcount + "/" + native.Params.Count);
                        native.Params.Clear();
                        for (var i = 0; i < pcount; ++i)
                            native.Params.Add(new Param("Any", "Param" + i));
                    }
                }
                else
                {
    ...
    {
        [JsonExtensionData] public IDictionary<string, JToken> Values { get; set; }
    }
    ...
    public class Native
    {
        ...
        [JsonProperty("jhash")] public string Joaat { get; set; }
        ...
        public IList<Param> Params
        {
            ...
            {
                _params = value;
                Vardiac = false; // Update vardiac condition
                foreach (var p in _params) Vardiac = Vardiac | p.Vardiac;
            }
        }

        [JsonProperty("hashes")] public IList<string> Hashes { get; set; }
         ...
         * Cached Fields
         */
        public bool Vardiac { get; private set; }
        ...
        {
            return index >= Params.Count && Vardiac ? VardiacParam : Params[index];
        }
        ...
        private void UpdateDirty()
        {
            ...
            var dispStr = Program.NativeName(Program.ShowNamespace && Namespace != "" ? Namespace + "::" : "");
            if (Name.StartsWith("_0x"))
                dispStr += Name.Remove(3) + Program.NativeName(Name.Substring(3));
            else if (Name.StartsWith(Program.NativeName(UnkPrefix)))
                dispStr += Name;
            else
    ...
    public class Param
    {
        ...
        public string Type
        {
            ...
            {
                Vardiac = value == "";
                _type = Vardiac ? "const char*" : value;
                if (X64NativeFile.TypeMap.ContainsKey(_type))
                {
                    StackType = X64NativeFile.TypeMap[_type];
                }
                else
                {
                    Console.WriteLine($"Unrecognized data type: {_type}");
                    StackType = Stack.DataType.Int;
                }
            }
        }
        ...
        /// <summary>
        /// </summary>
        public Stack.DataType StackType { get; private set; }
        ...
        /// <summary>
        /// </summary>
        public bool FixedType { get; private set; }

        public Param UpdateFixed()
        {
            FixedType = !StackType.IsUnknown();
            return this;
        }
    }
}
